...
class RuleBuilder(SchematronNodeBuilder, metaclass=ABCMeta):
    ...
    def add_checks(self, nodes: list[Check]):
        """Add a list of Check nodes (Rule or Assert).
    ...
    def add_attributes(self, element_attributes: dict[str, str], xpath_parser: Parser):
        """Add all the attributes of the XML Rule element in one go.

        This should be provided with an xpath parser to parse the `subject` attribute if present.

        Args:
            element_attributes: dictionary of attributes taken from the XML node
            xpath_parser: the xpath parser we use to parse the subject node if present
        """
        if 'context' in element_attributes:
            self.attributes['context'] = xpath_parser.parse(element_attributes['context'])

        if 'subject' in element_attributes:
            self.attributes['subject'] = xpath_parser.parse(element_attributes['subject'])
